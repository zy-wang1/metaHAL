...
sample_size <- 200  # n
p_image <- 200 * 4  # additional cov dimension; 200 * 4 here for high-d
ratio_overfit = 1  # this is by how much we overfit the glmnet; up to 0.1%
ratio_nonzero <- 0.005  # ratio of true non-zero coefficients from the additional covariates
truth <- trt_coef <- 1

for (ratio_overfit in c(1, 0.001)) {
  ...
  list_result <- mclapply(1:n_iter, mc.cores = n_cores, FUN = function(i_iter) {
    ...
    if (file.exists(output_target)) {
      print("This iteration was finished")
      ...
      record$noadj <- c(est_noadj)
      
      # full-data tmle with tmle3 functions
      {
        data_use <- data
        # setup
        {
          ...
          # choose base learners with current overfitting level
          lrnr_mean <- make_learner(Lrnr_mean)
          lrnr_glm <- make_learner(Lrnr_glm)
          temp_vec <- rep(1, ncol(data) - 2)
          temp_vec[grep("^X", colnames(data[, -ncol(data)]))] <- 0
          lrnr_glmnet_value_G <- Lrnr_glmnet_overfit_sparse$new(ratio_overfit = ratio_overfit, penalty.factor = temp_vec)
          temp_vec <- rep(1, ncol(data) - 1)
          temp_vec[grep("^X", colnames(data[, -ncol(data)]))] <- 0
          temp_vec[grep("^trt", colnames(data[, -ncol(data)]))] <- 0
          lrnr_glmnet_value_Q <- Lrnr_glmnet_overfit_sparse$new(ratio_overfit = ratio_overfit, penalty.factor = temp_vec)
        }
        {
          if (p_image < 10) {
            learner_list <- list(A = lrnr_glm, Y = lrnr_glm)  
          } else {
            learner_list <- list(A = lrnr_glmnet_value_G, Y = lrnr_glmnet_value_Q)  
          }
      ...
      record$tmle <- c(fit_summary_backup$init_est, fit_summary_backup$tmle_est)
      ...
      # metaHAL initial plugin
      {
        ...
        # setup
        {
          ...
          temp_vec[grep("^X", colnames(data[, -ncol(data)]))] <- 0
          temp_vec[grep("^trt", colnames(data[, -ncol(data)]))] <- 0
          lrnr_Q_1 <- Lrnr_glmnet_overfit_sparse_insert$new(trt_name = "trt", trt_value = 1, ratio_overfit = ratio_overfit, penalty.factor = temp_vec)
          lrnr_Q_0 <- Lrnr_glmnet_overfit_sparse_insert$new(trt_name = "trt", trt_value = 0, ratio_overfit = ratio_overfit, penalty.factor = temp_vec)
          
          temp_vec <- rep(1, ncol(data) - 2)
          temp_vec[grep("^X", colnames(data[, -ncol(data)]))] <- 0
          lrnr_G <- Lrnr_glmnet_overfit_sparse_G$new(force_DV = "trt", ratio_overfit = ratio_overfit, penalty.factor = temp_vec)
          ...
          if (p_image < 10) {
            ...
            temp_vec <- rep(1, ncol(data) - 2)
            temp_vec[grep("^X", colnames(data[, -ncol(data)]))] <- 0
            lrnr_G <- Lrnr_glm_insert_G$new(force_DV = "trt")
          }
  ...
  base_lrnrs <- list(lrnr_1, lrnr_2, lrnr_3, lrnr_4, lrnr_trt, lrnr_Q_1, lrnr_Q_0
      ...
      # undersmoothed metaHAL; 
      # G hat as the initial G in undersmoothing steps
      {
        {
      ...
      # undersmoothed metaHAL; step 2, reverse the lambda as much as possible by bisection, while keeping it solved; minimal overfitting
      {
        {
      ...
      # undersmooth with metaG
      {
        # fit metaG
        {
          temp_vec <- rep(1, ncol(data) - 2)
          temp_vec[grep("^X", colnames(data[, -ncol(data)]))] <- 0
          lrnr_treatment <- Lrnr_glmnet_overfit_sparse$new(ratio_overfit = ratio_overfit, penalty.factor = temp_vec)
