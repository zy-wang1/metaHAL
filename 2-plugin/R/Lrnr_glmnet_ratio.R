...
#'      \code{\link[glmnet]{cv.glmnet}}.
#'  - \code{ratio = 1}: If \code{use_min == FALSE}, the provide a ratio to the 
#'      selected lambda.
...
Lrnr_glmnet_ratio <- R6Class(
  classname = "Lrnr_glmnet_ratio",
  inherit = Lrnr_base, portable = TRUE, class = TRUE,
  public = list(
    initialize = function(lambda = NULL, type.measure = "deviance",
                          nfolds = 10, alpha = 1, nlambda = 100,
                          use_min = TRUE, ratio = 1, stratify_cv = FALSE, ...) {
  ...
  private = list(
    ...
    .predict = function(task) {
      ...
      if (self$params$use_min) {
        ...
        # args$s <- "lambda.1se"
        args$s <- args$object$lambda.min * ratio
      }
